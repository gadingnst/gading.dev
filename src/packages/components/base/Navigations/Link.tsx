...
export interface LinkProps extends NextLinkProps {
  variant?: 'default' | 'button' | 'glass' | 'ghost' | 'outline' | 'accent';
  size?: 'xs' | 'sm' | 'md' | 'lg';
  icon?: ReactNode;
...
/**
 * Reusable Link component with intuitive styling using DaisyUI 5
...
function Link(props: LinkProps) {
  ...
  const sizeClasses = {
    xs: variant === 'default' ? 'text-xs' : 'btn-xs',
    sm: variant === 'default' ? 'text-sm' : 'btn-sm',
    md: variant === 'default' ? 'text-base' : '',
    lg: variant === 'default' ? 'text-lg' : 'btn-lg'
  };

  const iconClasses = [
    'transition-transform duration-300',
    variant === 'default' && 'group-hover:translate-x-0.5'
  ];

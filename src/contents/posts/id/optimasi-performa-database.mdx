---
title: "Mengupas Performa Database: Pelajaran Mahal dari Homelab dan Production"
slug: {
  en: "optimizing-database-performance",
  id: "optimasi-performa-database"
}
date: 2026-06-01
description: "Cerita di balik layar optimasi database PostgreSQL. Dari drama connection leak di K3s, index overkill yang bikin bulk insert tewas, sampai cache basi di Redis."
...
Gue juga dulu sempat tergoda pake cara instan ini. Pas database PostgreSQL gue di LXC 103 (`db-host`) mulai ngos-ngosan karena dihajar request dari berbagai pod K3s dan workflow n8n, reaksi pertama gue adalah buka dashboard Proxmox, klik resource LXC, lalu naikin core CPU dari 2 jadi 4, dan RAM dari 2GB jadi 4GB.
...
Optimasi database itu bukan soal seberapa gede RAM server lu, tapi seberapa efisien engine database lu bekerja di balik layar. Berikut adalah beberapa \"bekas luka\" dan pelajaran mahal yang gue dapet dari ngoprek performa database di homelab dan di production tempat gue kerja sebelumnya.

---

### 1. Drama Connection Pooling (Ketika Kube Menyerang Postgres)

Ini kesalahan arsitektur paling pertama yang bikin gue kejedut. Di cluster K3s, gue pake Prisma ORM untuk beberapa microservices. Secara default, setiap kali pod aplikasi lu jalan, Prisma bakal nge-spawn connection pool sendiri (biasanya default 10-20 koneksi per instance).
...
*   **Transaction Mode vs Session Mode**: PgBouncer punya beberapa mode. Awalnya gue pasang *Transaction Mode* karena paling hemat koneksi. Eh, ternyata migrasi skema Prisma gue langsung rusak karena Prisma butuh *Session Mode* (yang nge-lock koneksi per sesi untuk fitur temporary tables/ad-hoc commands).
...
Semua orang tahu kalau query `SELECT` lambat, solusinya tinggal kasih index. Tapi gue pernah kejedut karena terlalu rajin pasang index.

Di satu tabel log transaksi n8n yang datanya masuk ribuan baris per hari, gue dengan songongnya masang index B-Tree di hampir semua kolom yang sering gue filter (`status`, `created_at`, `user_id`, `amount`, `gateway`). 

#### Akibatnya: Bulk Insert Tewas
Query baca emang jadi instan (di bawah 5ms). Tapi pas n8n mulai nge-run workflow bulk insert ribuan data transaksi sekaligus, CPU server database langsung spikes 100% dan proses insert-nya jadi lambat banget kayak siput. 
...
Query database paling cepat adalah query yang gak pernah lu jalankan. Di tech company tempat gue kerja sebelumnya, gue pasang **Redis** sebagai caching layer di depan database untuk data yang sering dibaca tapi jarang berubah (seperti daftar produk atau konfigurasi sistem).
...
Tapi caching ini ada jebakan mautnya: *Cache Invalidation*. Gue pernah lupa nge-set TTL (Time-To-Live) dan gak nge-clear cache Redis pas ada transaksi baru untuk saldo token NOUVA user. 

Alhasil, user udah bayar via Xendit dan saldo di Postgres udah nambah, tapi di dashboard dashboard mereka saldonya tetep nol karena aplikasi masih ngebaca data lama dari Redis. User langsung panik dan komplain ke CS.
...
Database itu jantung dari aplikasi lu. Rawat dia dengan query yang efisien dan arsitektur koneksi yang rapi, bukan dengan asal geser slider kapasitas RAM di dashboard cloud provider lu. wkwk
